    ...
    class RT_DB_Update {
        ...
        public function __construct( $plugin_path = false, $schema_path = false, $mu_single_table = false, $current_version = false ) {
            ...
            if( !file_exists( $this->schema_path ) ) {
                if( $this->debugMode ) {
                    $message = sprintf( __( 'The Path %s does not point to a valid schema directory.', 'buddy-views' ), $this->schema_path );
            ...
            if( !file_exists( $this->plugin_path ) ) {
                if( $this->debugMode ) {
                    $message = sprintf( __( 'The Path %s does not point to a valid plugin.', 'buddy-views' ), $this->plugin_path );
                    trigger_error( $message, E_USER_WARNING );
                }
            }
