    ...
    class Rt_Gravity_Reports {
        ...
        static function generate_data_source( $form_id, $chart_type, $selected_fields, $labels = array() ) {
            ...
            switch( $chart_type ) {
                ...
                case 'gauge':
                    ...
                    $cols = array( $field[ 'label' ], __( 'Count', 'buddy-views' ) );
                ...
                case 'stepped_area':
                    ...
                    foreach( $leads as $lead ) {
                        ...
                        if( !empty( $temp ) ) {
                            ...
                            $counts[] = array(
                                'label' => __( ( isset( $user->display_name ) ? $user->display_name : 'Annonymous' ) . ' - ' . $date->format( 'j M Y' ), 'buddy-views' ),
                                'count' => $temp,
                            );
                            $temp = array();
                        }
        ...
        static function get_form_fields( $form_id, $field_type = 'others' ) {
            ...
            if( 'range' == $field_type ) {
                $return_fields[] = array(
                    'id' => '-1',
                    'label' => __( 'Form Entry Date', 'buddy-views' ),
                    'type' => 'range',
                );
            }
